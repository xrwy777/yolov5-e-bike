...
# SPP-Fast顾名思义就是为了保证准确率相似的条件下减少计算量，以提高速度，使用3个5×5的最大池化，代替原来的5×5、9×9、13×13最大池化。
...
'''
这两个函数是用于改变feature map的维度,并不是很常用。
...
'''
这是一个二级分类模块，什么是二级分类模块？比如做车牌的识别，先识别出车牌 ，如果想对车牌上的字进行识别，
就需要二级分类进一步检测。如果对模型输出的分类再进行分类，就可以用这个模块。不过这里这个类写的比较简单，
若进行复杂的二级分类，可以根据自己的实际任务可以改写，这里代码不唯一。
这里的功能和torch_utils.py中的load_classifier函数功能相似,不
过我们这里是给模型自定义的，可以直接加载在模型上。用的不多的。
'''
